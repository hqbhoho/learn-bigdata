...
public class HiveCliDriver {
    ...
    public int processCmd(String cmd) {
        ...
        if (cmd_trimmed.toLowerCase().equals("quit") || cmd_trimmed.toLowerCase().equals("exit")) {
            ...
            try {
                CommandProcessor proc = Factory.get(tokens, (HiveConf) conf);
                if (proc instanceof HiveDriver) {
     ...
     */
    private void printHeader(HiveDriver qp, PrintStream out) {
...
//        String keytab_path = "/etc/security/keytabs/hive-kylo.headless.keytab";
    ...
    public  int run(String[] args) throws Exception {
        ...
        conf.setVar(HiveConf.ConfVars.POSTEXECHOOKS, LineageLogger.class.getName());
    ...
    private int executeDriver(CliSessionState ss, HiveConf conf, OptionsProcessor oproc)
        ...
        // TODO while ((line = reader.readLine(curPrompt + "> ")) != null) {
        while (this.sqls.size() > 0) {
            line = this.sqls.pollFirst();
            if (!prefix.equals("")) {
                prefix += '\n';
            }
