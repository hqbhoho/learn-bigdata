...
import com.hqbhoho.bigdata.hive.parse1.hooks.LineageInfoToDBHook;
...
public class HiveCliDriver {
    ...
    public HiveCliDriver() {
        SessionState ss = SessionState.get();
        conf = (ss != null) ? ss.getConf() : new Configuration();
        Log LOG = LogFactory.getLog("HiveCliDriver");
    ...
    public int processCmd(String cmd) {
        ...
        if (cmd_trimmed.toLowerCase().equals("quit") || cmd_trimmed.toLowerCase().equals("exit")) {
            ...
            try {
                // TODO CommandProcessorFactory -> HiveCommandProcessorFactory
                CommandProcessor proc = HiveCommandProcessorFactory.get(tokens, (HiveConf) conf);
                if (proc instanceof HiveProcessDriver) {
    ...
    int processLocalCmd(String cmd, CommandProcessor proc, CliSessionState ss) {
        ...
        do {
            try {
                needRetry = false;
                if (proc != null) {
                    // TODO Driver -> HiveProcessDriver
                    if (proc instanceof HiveProcessDriver) {
                        HiveProcessDriver qp = (HiveProcessDriver) proc;
     ...
     * for the output.
     *
     * @param qp  HiveProcessDriver that executed the command
     * @param out PrintStream which to send output to
     */
    private void printHeader(HiveProcessDriver qp, PrintStream out) {
    ...
    public int processReader(BufferedReader r) throws IOException {
        ...
        // todo  增加过滤条件 忽略set语句以及注释语句 增加多行注释标识
        boolean flag = true;
    ...
    // TODO 这里增加一个判断逻辑  判断filename是文件夹还是目录
    public int processFileCustom(String fileName) throws Exception {
        File source = new File(fileName);
        List<String> paths = FileUtils.traversalFileDirectory(source);
    ...
    public int processFile(String fileName) throws IOException {
        ...
        try {
            // TODO 添加解析的sql脚本的路径
            this.conf.set("script_path", path.toString());
    ...
    public static void main(String[] args) throws Exception {
        int ret = new HiveCliDriver().run(args);
        System.exit(ret);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        // TODO add LineageInfoToDBHook
        conf.setVar(HiveConf.ConfVars.POSTEXECHOOKS, LineageLogger.class.getName() + "," + LineageInfoToDBHook.class.getName());
    ...
    private int executeDriver(CliSessionState ss, HiveConf conf, OptionsProcessor oproc)
        ...
        HiveCliDriver cli = new HiveCliDriver();
        ...
        // TODO use processFileCustom() to process whether is file or not
        try {
            if (ss.fileName != null) {
//                return cli.processFile(ss.fileName);
                return cli.processFileCustom(ss.fileName);
            } else {
