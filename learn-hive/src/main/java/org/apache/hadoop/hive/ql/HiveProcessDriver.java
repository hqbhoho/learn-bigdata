 ...
 * @date 2020/05/19
 */
public class HiveProcessDriver implements CommandProcessor {

    static final private String CLASS_NAME = HiveProcessDriver.class.getName();
    ...
    private Driver.LockedDriverState lDrvState = new Driver.LockedDriverState();
    ...
    // TODO USE Driver.DriverState  2020-05-21
...
*/
    // TODO USE Driver.LockedDriverState   2020-05-21
     ...
     */
    public HiveProcessDriver(HiveConf conf) {
        this(conf, new HooksLoader(conf));
    }

    public HiveProcessDriver(HiveConf conf, String userName) {
        this(conf, userName, new HooksLoader(conf), null);
    }

    public HiveProcessDriver() {
        this((SessionState.get() != null) ? SessionState.get().getConf() : null);
    }

    public HiveProcessDriver(HiveConf conf, HooksLoader hooksLoader) {
        this(conf, null, hooksLoader, null);
    }

    public HiveProcessDriver(HiveConf conf, String userName, QueryInfo queryInfo) {
        this(conf, userName, new HooksLoader(conf), queryInfo);
    }

    public HiveProcessDriver(HiveConf conf, String userName, HooksLoader hooksLoader, QueryInfo queryInfo) {
        this.conf = conf;
    ...
    public int compile(String command, boolean resetTaskIds, boolean deferClose) {
        ...
        Driver.LockedDriverState.setLockedDriverState(lDrvState);
    ...
    private boolean isInterrupted() {
        lDrvState.stateLock.lock();
        try {
            if (lDrvState.driverState == Driver.DriverState.INTERRUPT) {
                return true;
            } else {
                return false;
            }
    ...
    /**
     * @param d    The database to be locked
     * @param t    The table to be locked
     * @param p    The partition to be locked
     * @param mode The mode of the lock (SHARED/EXCLUSIVE) Get the list of objects to be locked. If a
    ...
    private CommandProcessorResponse runInternal(String command, boolean alreadyCompiled)
        ...
        try {
            if (alreadyCompiled) {
                if (lDrvState.driverState == Driver.DriverState.COMPILED) {
                    lDrvState.driverState = Driver.DriverState.EXECUTING;
                } else {
        ...
        try {
            ...
            try {
                if (lDrvState.driverState == Driver.DriverState.INTERRUPT) {
                    lDrvState.driverState = Driver.DriverState.ERROR;
                } else {
                    lDrvState.driverState = isFinishedWithError ? Driver.DriverState.ERROR : Driver.DriverState.EXECUTED;
                }
            } finally {
                lDrvState.stateLock.unlock();
            }
        ...
        try {
            ...
            if (lDrvState.driverState != Driver.DriverState.COMPILED &&
                ...
                errorMessage = "FAILED: query " + queryStr + " has " +
                        (lDrvState.driverState == Driver.DriverState.INTERRUPT ? "been cancelled" : "not been compiled.");
                console.printError(errorMessage);
                return 1000;
            } else {
                lDrvState.driverState = Driver.DriverState.EXECUTING;
        ...
        try {
            ...
            try {
                if (isInterrupted) {
                    ...
                    lDrvState.driverState = executionError ? Driver.DriverState.ERROR : Driver.DriverState.EXECUTED;
                }
            } finally {
                lDrvState.stateLock.unlock();
            }
    ...
    private TaskRunner launchTask(Task<? extends Serializable> tsk, String queryId, boolean noName,
        ...
        // Launch Task
        //  TODO modify task run mode,will skip some task ,like MapRedTask etc
        if (HiveConf.getBoolVar(conf, HiveConf.ConfVars.EXECPARALLEL) && tsk.isMapRedTask()) {
            ...
            if (LOG.isInfoEnabled()) {
                LOG.info("lineage analyzer ,will skip task [" + tsk + "] in serial mode");
            }
            tskRun.getTaskResult().setExitVal(0, null);
            tskRun.getTaskResult().setRunning(false);
            tskRun.getTask().setDone();
        } else {
            ...
            // skip some task
            if (tskRun.getTask() instanceof ExecDriver ||
                    tskRun.getTask() instanceof MapredLocalTask ||
                    tskRun.getTask() instanceof StatsTask ||
                    tskRun.getTask() instanceof DDLTask) {
                if (LOG.isInfoEnabled()) {
                    LOG.info("lineage analyzer ,will skip task [" + tsk + "] in serial mode");
                }
                tskRun.getTaskResult().setExitVal(0, null);
    ...
    public boolean getResults(List res) throws IOException, CommandNeedRetryException {
        if (lDrvState.driverState == Driver.DriverState.DESTROYED || lDrvState.driverState == Driver.DriverState.CLOSED) {
    ...
    public int close() {
        ...
        try {
            releaseDriverContext();
            if (lDrvState.driverState == Driver.DriverState.COMPILING ||
                    lDrvState.driverState == Driver.DriverState.EXECUTING ||
                    lDrvState.driverState == Driver.DriverState.INTERRUPT) {
                lDrvState.driverState = Driver.DriverState.INTERRUPT;
                return 0;
            }
            releasePlan();
            ...
            releaseResStream();
            releaseContext();
            lDrvState.driverState = Driver.DriverState.CLOSED;
        } finally {
            lDrvState.stateLock.unlock();
            Driver.LockedDriverState.removeLockedDriverState();
        }
        if (SessionState.get() != null) {
            SessionState.get().getLineageState().clear();
        }
    ...
    public void destroy() {
        ...
        try {
            ...
            // the query process
            if (lDrvState.driverState == Driver.DriverState.DESTROYED ||
                    lDrvState.driverState == Driver.DriverState.INTERRUPT) {
                return;
            } else {
                lDrvState.driverState = Driver.DriverState.DESTROYED;
            }
        } finally {
            lDrvState.stateLock.unlock();
        }
